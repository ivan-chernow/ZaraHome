...
describe('ProductsController', () => {
  ...
  describe('findAll', () => {
    it('возвращает пагинированный список с фильтрами и сортировкой', async () => {
      ...
      expect(result).toEqual({
        data: serviceResult,
        message: PRODUCTS_CONSTANTS.SUCCESS.PRODUCTS_LOADED,
        success: true,
      });
    });
    ...
    it('использует дефолтные значения пагинации при отсутствии page/limit', async () => {
      ...
      const serviceResult = {
        ...
        total: 0,
        page: 1,
        limit: PRODUCTS_CONSTANTS.DEFAULT_PAGE_SIZE,
        totalPages: 0,
        hasNext: false,
        hasPrev: false,
      };
  ...
  describe('searchProducts', () => {
    it('возвращает найденные продукты', async () => {
      ...
      expect(result).toEqual({
        data: items,
        message: PRODUCTS_CONSTANTS.SUCCESS.SEARCH_COMPLETED,
        success: true,
      });
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('бросает ResourceNotFoundException если не найден', async () => {
      ...
      await expect(controller.findOne({ id: 1 } as any)).rejects.toBeInstanceOf(
