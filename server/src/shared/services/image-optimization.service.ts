...
export class ImageOptimizationService implements IImageOptimizationService {
  ...
  async ensureOutputDir(): Promise<void> {
    try {
      ...
      throw new InternalServerErrorException(
        SHARED_CONSTANTS.ERRORS.GENERAL.DIRECTORY_CREATION_FAILED
      );
    }
  }
    ...
    try {
      ...
      if (!metadata.width || !metadata.height) {
        throw new BadRequestException(
          SHARED_CONSTANTS.ERRORS.GENERAL.INVALID_IMAGE
        );
      }

      // Проверяем минимальные размеры
      if (
        metadata.width < SHARED_CONSTANTS.IMAGES.MIN_DIMENSIONS ||
        metadata.height < SHARED_CONSTANTS.IMAGES.MIN_DIMENSIONS
      ) {
        throw new BadRequestException(
      ...
      if (
        metadata.width > SHARED_CONSTANTS.IMAGES.MAX_DIMENSIONS ||
        metadata.height > SHARED_CONSTANTS.IMAGES.MAX_DIMENSIONS
      ) {
        throw new BadRequestException(
      ...
      throw new BadRequestException(
        SHARED_CONSTANTS.ERRORS.GENERAL.INVALID_IMAGE_FORMAT
      );
    }
  }
  ...
  async processAndSave(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        SHARED_CONSTANTS.ERRORS.GENERAL.IMAGE_PROCESSING_FAILED
      );
    }
  }
  ...
  async generateThumbnail(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        SHARED_CONSTANTS.ERRORS.GENERAL.OPERATION_FAILED
      );
    }
  }
  ...
  async cleanupOldImages(
    maxAge: number = SHARED_CONSTANTS.IMAGES.DEFAULT_CLEANUP_AGE
  ): Promise<number> {
    try {
