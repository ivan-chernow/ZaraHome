...
export class MonitoringService implements IMonitoringService {
  ...
  private readonly metrics = new Map<
    string,
    { value: number; timestamp: number; tags: Record<string, string> }
  >();
    ...
    try {
      ...
      return {
        ...
        checks: Object.fromEntries(
          checks.map(check => [
            check.name,
            { status: check.status, responseTime: check.responseTime },
          ])
        ),
        timestamp: new Date(),
  ...
  async recordApiCall(
    ...
    await this.recordMetric('api.calls', 1, {
      endpoint,
      method,
      status: statusCode.toString(),
    });
    await this.recordTiming('api.response_time', duration, {
      endpoint,
      method,
    });

    if (statusCode >= 400) {
      await this.incrementCounter('api.errors', {
        endpoint,
        method,
        status: statusCode.toString(),
      });
  ...
  async recordDatabaseQuery(
    ...
    await this.recordTiming('database.query_time', duration, {
      query: query.substring(0, 50),
    });
    await this.incrementCounter('database.queries', {
      success: success.toString(),
    });

    if (!success) {
      await this.incrementCounter('database.errors', {
        query: query.substring(0, 50),
      });
    }
  }
  ...
  async recordCacheOperation(
    ...
    await this.recordTiming('cache.operation_time', duration, {
      operation,
      key: key.substring(0, 50),
    });
    await this.incrementCounter('cache.operations', {
      operation,
      hit: hit.toString(),
    });
