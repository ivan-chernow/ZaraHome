...
export const TypePageContent: React.FC<TypePageContentProps> = ({ params }) => {
  ...
  // Сброс страницы только если тип реально меняется
  useEffect(() => {
    if (prevType.current && prevType.current !== typeSlug) {
      // Сбрасываем страницу в URL
      const params = new URLSearchParams(searchParams);
      params.delete('page');
      router.push(`?${params.toString()}`, { scroll: false });
    }
    prevType.current = typeSlug;
  }, [typeSlug, router, searchParams]);

  const customSlugify = (text: string) =>
    slugify(text.replace('й', 'y'), { lower: true, strict: true });

  // Получаем продукты типа
  const baseProducts = useMemo(() => {
    ...
    const currentType = currentSubCategory.types?.find(
      type => customSlugify(type.name) === typeSlug
    );
    if (!currentType) return [];

    return getProductsByType(currentType);
  }, [categories, categorySlug, subCategorySlug, typeSlug]);
  ...
  const typeName = useMemo(() => {
    if (!categories) return decodeURIComponent(typeSlug);
    const currentCategory = categories.find(
      cat => customSlugify(cat.name) === categorySlug
    );
    if (!currentCategory) return decodeURIComponent(typeSlug);

    const currentSubCategory = currentCategory.subCategories?.find(
      sub => customSlugify(sub.name) === subCategorySlug
    );
    if (!currentSubCategory) return decodeURIComponent(typeSlug);

    const currentType = currentSubCategory.types?.find(
      type => customSlugify(type.name) === typeSlug
    );
    return currentType ? currentType.name : decodeURIComponent(typeSlug);
  }, [categories, categorySlug, subCategorySlug, typeSlug]);
