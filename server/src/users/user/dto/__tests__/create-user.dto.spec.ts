...
describe('CreateUserDto', () => {
  it('валидно при корректных данных', async () => {
    const { errors } = await build({
      ...
      password: 'a'.repeat(USERS_CONSTANTS.MIN_PASSWORD_LENGTH),
    });
  ...
  it.each([
    ...
    {
      email: 'user@example.com',
      password: 'a'.repeat(USERS_CONSTANTS.MAX_PASSWORD_LENGTH + 1),
    },
    {
      email: 'x'.repeat(USERS_CONSTANTS.MAX_EMAIL_LENGTH + 1) + '@e.com',
      password: 'goodpass',
    },
  ...
  it('опциональные строки валидируются по длине', async () => {
    ...
    const { errors } = await build({
      ...
      firstName: long,
      lastName: long,
      defaultAddress: 'x'.repeat(USERS_CONSTANTS.MAX_ADDRESS_LENGTH + 1),
    });
    expect(errors.length).toBeGreaterThan(0);
  });
