...
describe('OrdersService (unit)', () => {
  ...
  it('createOrder: валидирует, учитывает активный заказ, считает тоталы и применяет промокод', async () => {
    ...
    promocodes.validateAndApply.mockResolvedValue({ isValid: true, finalAmount: 225, discount: 25 });
    repo.createOrder.mockResolvedValue({ id: 10, totalPrice: 225, totalCount: 3 });
  ...
  it('createOrder: должен отменить существующий активный заказ при изменении товаров', async () => {
    ...
    const existingOrder = { id: 5, status: 'PENDING', items: [{ productId: 1, quantity: 1, price: 100 }] };
    
    repo.findUserById.mockResolvedValue({ id: 1 });
    repo.findActiveOrderByUser.mockResolvedValue(existingOrder);
    repo.updateOrder.mockResolvedValue({ ...existingOrder, status: 'CANCELLED' });
    repo.createOrder.mockResolvedValue({ id: 10, totalPrice: 250, totalCount: 3 });

    await service.createOrder(dto, 1);
    
    expect(repo.updateOrder).toHaveBeenCalledWith(expect.objectContaining({ status: 'cancelled' }));
  ...
  it('createOrder: должен обработать невалидный промокод', async () => {
    ...
    repo.findUserById.mockResolvedValue({ id: 1 });
    repo.findActiveOrderByUser.mockResolvedValue(null);
    promocodes.validateAndApply.mockResolvedValue({ isValid: false, message: 'Промокод не найден' });
    repo.createOrder.mockResolvedValue({ id: 10, totalPrice: 200, totalCount: 2 });
  ...
  it('getUserOrders: некорректная пагинация', async () => {
    await expect(service.getUserOrders(1, 0, 10)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('searchOrders: короткая строка', async () => {
    await expect(service.searchOrders('a')).rejects.toBeInstanceOf(BadRequestException);
  });

  it('updateOrderStatus: валидирует переход статуса', async () => {
    repo.findOrderByIdAndUser.mockResolvedValue({ id: 1, status: 'delivered' });
    await expect(service.updateOrderStatus(1, 'paid' as any, 1)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('getUserOrders: успешный вызов репозитория', async () => {
    repo.findOrdersByUser.mockResolvedValue({ orders: [], total: 0, page: 1, limit: 10, totalPages: 0 });
