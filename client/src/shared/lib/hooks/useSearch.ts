...
export const useSearch = <T extends Record<string, any>>(
  ...
  const createSearchIndex = useCallback(
    (items: T[]) => {
  ...
  const performSearch = useCallback(
    (query: string) => {
      ...
      const searchWords = searchQuery
        .split(/\s+/)
        .filter(word => word.length >= minSearchLength);
...
export const useSearchWithHistory = <T extends Record<string, any>>(
  ...
  const searchHook = useSearch(data, searchOptions);
  const { maxHistoryItems = 10, storageKey = 'search-history' } =
    historyOptions;
  ...
  const searchWithHistory = useCallback(
    (query: string) => {
