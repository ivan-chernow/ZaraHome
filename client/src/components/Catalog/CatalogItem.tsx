...
const CatalogItem = ({ card }: { card: { category: string, subtitle: string, img: string, link: string } }) => {
		...
		>
			<Link href={`/products/category/${categorySlug}`}>
				<div className='bg-white w-[337px] h-[189px] flex flex-col items-center justify-center mt-[370px]'>
					...
					<p className="text-[18px] mt-[9px] max-w-[231px] text-center">{card.subtitle}</p>
				</div>
			</Link>
		</li>
	);
