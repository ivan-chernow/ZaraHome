...
export const CartPageContent: React.FC = () => {
	...
	const { data: productsByIds, isLoading: isLoadingProducts, isError: isProductsError } = useGetProductsByIdsQuery(ids, {
		skip: !mounted || ids.length === 0,
		refetchOnFocus: true,
		refetchOnReconnect: true,
	});
	const safeProducts: Product[] = Array.isArray(productsByIds) ? productsByIds : [];
	const idToProduct: Record<number, Product> = safeProducts.reduce((acc, p) => {
		acc[p.id] = p;
		return acc;
	...
	const haveItemsChanged = useCallback((currentItems: any[], newItems: any[]): boolean => {
		...
		const createItemKey = (item: any) => `${item.productId}-${item.quantity}-${item.size ?? ''}-${item.color ?? ''}`;
		...
		<section className="pb-[101px] pt-[45px]">
			<Container maxWidth="lg">
				<div className="flex items-center mb-[31px]">
					<Link href="/">
						<HomeOutlinedIcon fontSize="small" className="cursor-pointer hover:opacity-80 transition" />
					</Link>
						...
						{!mounted ? (
							...
							(() => {
								...
								const handleDec = (item: CartItemType) =>
									dispatch(removeCartItem({ id: item.id, size: item.size, color: item.color }));
								...
								const handleSetQty = (item: CartItemType, qty: number) =>
									dispatch(setCartItemQuantity({ id: item.id, size: item.size, color: item.color, quantity: qty }));
														...
														<div className="flex flex-col min-w-0">
															{isLoadingProducts && !hasProductData ? (
																...
																<>
																	...
																	<p className="font-medium text-[#00000080] text-[12px] leading-4 truncate">{isProductsError ? 'Ошибка загрузки' : 'Название недоступно'}</p>
																	<div className="mt-1 text-[12px] text-[#00000099]">Всего: {totalQty} шт.</div>
																</>
															)}
													</div>
													...
													<div key={`${v.id}-${v.size ?? ''}-${v.color ?? ''}`} className="flex items-center justify-between py-1">
														<div className="text-[12px] text-[#00000099] flex items-center gap-4">
															...
															<span className="flex items-center gap-1">
																Цвет:
																<span className="inline-block w-3 h-3 rounded-full border" style={{ backgroundColor: (product?.colors?.[v.color ?? ''] ?? v.color ?? 'transparent') as string }} />
															</span>
														</div>
													<div className="flex items-center gap-3">
														<button aria-label="Уменьшить количество" onClick={() => handleDec(v)} className="w-7 h-7 flex items-center justify-center rounded-full bg-[#F2F2F2] text-[#6B7280] hover:bg-[#E5E7EB] transition transform hover:scale-105 active:scale-95 cursor-pointer leading-none">
															<RemoveRoundedIcon fontSize="small" />
														</button>
														<input className="w-[56px] h-[30px] border border-[#E5E5E5] text-center text-[14px] rounded outline-none focus:border-gray-400" inputMode="numeric" pattern="[0-9]*" value={String(v.quantity)} onChange={e => {
														...
														<button aria-label="Увеличить количество" onClick={() => handleInc(v)} className="w-7 h-7 flex items-center justify-center rounded-full bg-[#F2F2F2] text-[#6B7280] hover:bg-[#E5E7EB] transition transform hover:scale-105 active:scale-95 cursor-pointer leading-none">
															<AddRoundedIcon fontSize="small" />
														</button>
														<button aria-label="Удалить вариант" onClick={() => handleDel(v)} className="ml-1 p-1 rounded-full bg-[#F2F2F2] text-[#6B7280] hover:bg-[#E5E7EB] transition-colors duration-150 transform hover:scale-105 active:scale-95 cursor-pointer">
						...
						{!mounted ? (
							...
							<>
								...
								<p className="font-medium mb-[28px]">
									{totalCount}{' '}
									{totalCount === 1 ? 'товар' : totalCount >= 2 && totalCount <= 4 ? 'товара' : 'товаров'}
								</p>
								<div>
									{!isAuthenticated && (
										...
										onClick={async () => {
											...
											try {
												if (activeOrder && activeOrder.items) {
													...
													const newItems = cartItems.map(item => ({ productId: item.id, quantity: item.quantity, size: item.size, color: item.color }));
												...
												const orderData = {
													items: cartItems.map(item => {
														...
														return { productId: item.id, productName, quantity: Math.max(1, Math.floor(item.quantity || 1)), price, size: item.size, color: item.color };
													}),
												} as const;
									...
									{!isAuthenticated && (
										<div className="mt-2 text-center text-sm">
											<button className="underline cursor-pointer text-[#00000099]" onClick={() => { dispatch(setView('signup')); dispatch(openModalAuth()); }}>Зарегистрироваться</button>
										</div>
									)}
								</div>
							</>
