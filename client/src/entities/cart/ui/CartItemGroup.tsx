...
const CartItemGroup = memo<CartItemGroupProps>(function CartItemGroup({
  ...
  const handleDec = useCallback((item: CartItemType) => 
    dispatch(removeCartItem({ id: item.id, size: item.size, color: item.color })), 
    [dispatch]
  );
  
  const handleInc = useCallback((item: CartItemType) => 
    dispatch(addCartItem({ id: item.id, price: item.price, img: item.img, size: item.size, color: item.color })), 
    [dispatch]
  );
  
  const handleDel = useCallback((item: CartItemType) => 
    dispatch(deleteCartItem({ id: item.id, size: item.size, color: item.color })), 
    [dispatch]
  );
  
  const handleSetQty = useCallback((item: CartItemType, qty: number) => 
    dispatch(setCartItemQuantity({ id: item.id, quantity: qty, size: item.size, color: item.color })), 
    [dispatch]
  );
      ...
      <li className="flex flex-col py-4">
        ...
        <div className="mt-3 ml-[95px] flex flex-col gap-3">
          {variants.map((variant) => (
            <div key={`${variant.id}-${variant.size ?? ''}-${variant.color ?? ''}`} className="flex items-center justify-between py-1">
              <div className="text-[12px] text-[#00000099] flex items-center gap-4">
                <span>
                  {(() => {
                    const firstSizeKey = finalProduct?.size ? Object.keys(finalProduct.size)[0] : undefined;
                    const resolvedKey = (variant.size && variant.size !== '') ? variant.size! : (firstSizeKey ?? '');
                    const resolvedLabel = resolvedKey ? (finalProduct?.size?.[resolvedKey]?.size ?? resolvedKey) : '-';
                ...
                <span className="flex items-center gap-1">
                  ...
                  <span
                    ...
                    style={{ backgroundColor: (finalProduct?.colors?.[variant.color ?? ''] ?? variant.color ?? 'transparent') as string }}
                  />
                </span>
              </div>
