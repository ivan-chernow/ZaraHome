...
import { useGetAllActiveQuery, useCreatePromocodeMutation, useDeactivatePromocodeMutation } from '@/api/promocodes.api';
...
const Promocodes = () => {
  ...
  const { data: promocodes, isLoading } = useGetAllActiveQuery();
  const [createPromocode, { isLoading: isCreating, error: createError }] = useCreatePromocodeMutation();
  ...
  return (
    <Fade in={true} timeout={700}>
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <h4 className="font-light text-[42px] mb-[37px]">Управление промокодами</h4>

        <div className='flex items-center mb-[28px]'>
          <p className="font-medium text-[#0000004D] mr-[5px]">Создание промокода</p>
        ...
        <form onSubmit={handleCreatePromocode} className="mb-[37px]">
          <div className="flex items-center mb-[23px]">
            <div className="flex flex-col mr-[27px]">
              <label htmlFor="code" className='mb-[5px] pl-[20px] font-medium text-[14px] text-[#00000099]'>
                Код промокода
              </label>
              <TextField
                id="code"
            ...
            <div className="flex flex-col">
              <label htmlFor="discount" className='mb-[5px] pl-[20px] font-medium text-[14px] text-[#00000099]'>
                Скидка (%)
              </label>
              <TextField
                id="discount"
        ...
        {/* Таблица промокодов */}
        <div className='flex items-center mb-[28px]'>
          <p className="font-medium text-[#0000004D] mr-[5px]">Список промокодов</p>
          <HorizontalLine width='615px' />
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#00000099] uppercase tracking-wider">Код</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#00000099] uppercase tracking-wider">Скидка</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#00000099] uppercase tracking-wider">Статус</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#00000099] uppercase tracking-wider">Дата создания</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#00000099] uppercase tracking-wider">Действия</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {promocodes?.map((promo) => (
                  <tr key={promo.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#00000099]">{promo.code}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[#00000099]">{promo.discount}%</td>
