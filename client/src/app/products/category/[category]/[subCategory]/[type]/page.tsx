...
const Page = ({ params }: { params: Promise<{ category: string, subCategory: string, type: string }> }) => {
  const { category: categorySlug, subCategory: subCategorySlug, type: typeSlug } = use(params);
  ...
  const currentSubCategory = currentCategory
    ? currentCategory.subCategories.find(subCat => customSlugify(subCat.name) === subCategorySlug)
    : undefined;
  const currentType = currentSubCategory 
    ? currentSubCategory.types.find(type => customSlugify(type.name) === typeSlug)
    : undefined;
  
  const products = getProductsByType(currentType);
  ...
  const typeName = currentType ? currentType.name : decodeURIComponent(typeSlug);
  ...
  return (
    <div className="flex flex-col flex-1 pl-0">
      <div className="flex items-center mb-[39px]">
        ...
        <span className="mx-[6px] text-[#0000004D]">{'>'}</span>
        <Link href={`/products/category/${categorySlug}/${subCategorySlug}`} className="text-[#00000099] text-[14px] font-medium hover:underline">
            {subCategoryName}
        </Link>
        <span className="mx-[6px] text-[#0000004D]">{'>'}</span>
