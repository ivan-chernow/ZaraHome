...
export class RateLimitGuard implements CanActivate {
  ...
  private getRateLimitOptions(context: ExecutionContext): RateLimitOptions | null {
    ...
    return {
      ...
      maxRequests: options.maxRequests || SHARED_CONSTANTS.SECURITY.RATE_LIMIT_MAX_REQUESTS,
  ...
  getStats(): Record<string, any> {
    ...
    const activeKeys = Array.from(this.requestCounts.entries())
      ...
      .map(([key, data]) => ({
        ...
        remaining: Math.max(0, SHARED_CONSTANTS.SECURITY.RATE_LIMIT_MAX_REQUESTS - data.count)
      }));
