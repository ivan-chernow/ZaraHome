import { Injectable, NotFoundException, OnModuleInit, Logger, BadRequestException, InternalServerErrorException } from '@nestjs/common';
...
export class AdminService implements OnModuleInit, IAdminService {
    ...
    async addProduct(files: Express.Multer.File[], productData: CreateProductDto) {
        ...
        this.logger.log(`Начинаем создание продукта с ${files?.length || 0} файлами`);
        ...
        try {
                    ...
                    const retryResult = await this.errorHandlerService.processWithRetry(
                        ...
                        async (fileToProcess) => {
                            try {
                                ...
                                this.logger.error(`Ошибка обработки изображения ${fileToProcess.originalname}:`, error.message);
                                throw error;
                            }
                        },
            ...
            this.logger.log(`Успешно обработано ${imagePaths.length} изображений, создаем продукт`);
            ...
            this.logger.log(`✅ Продукт успешно создан: ID ${product.id} с ${imagePaths.length} изображениями`);
            this.logger.log(`📊 Статистика загрузки: ${stats.successful}/${stats.total} файлов обработано успешно (${Math.round(stats.successRate * 100)}%)`);
            ...
            this.uploadMonitoringService.recordUploadResults(uploadResult.results, uploadTime);
            ...
            this.logger.error(`❌ Критическая ошибка при создании продукта:`, error.message);
