...
describe('ResetPasswordService (unit)', () => {
  ...
  describe('requestReset', () => {
    it('проверяет пользователя, ограничивает частоту, сохраняет токен и шлёт письмо', async () => {
      userRepository.findOne.mockResolvedValue({ id: 1, email: 'a@a.a', isEmailVerified: true });
      ...
      expect(resetRepository.save).toHaveBeenCalledWith(expect.objectContaining({
        email: 'a@a.a',
        token: expect.any(String),
        expiresAt: expect.any(Date),
        createdAt: expect.any(Date),
      }));
      expect(emailService.sendResetPasswordEmail).toHaveBeenCalledWith('a@a.a', expect.any(String));
    });

    it('ошибка: пользователь не найден/не верифицирован', async () => {
      userRepository.findOne.mockResolvedValue(null);
      await expect(service.requestReset('no@a.a')).rejects.toBeInstanceOf(BadRequestException);
    });

    it('ошибка: rate-limit 15 минут', async () => {
      userRepository.findOne.mockResolvedValue({ id: 1, email: 'a@a.a', isEmailVerified: true });
      resetRepository.findOne.mockResolvedValue({ createdAt: new Date() });
      await expect(service.requestReset('a@a.a')).rejects.toBeInstanceOf(BadRequestException);
    });
  });
  ...
  describe('verifyCode', () => {
    it('валидный токен', async () => {
      ...
      expect(token).toBe('t');
      expect(resetRepository.save).toHaveBeenCalledWith(expect.objectContaining({ isVerified: true }));
    });
  ...
  describe('setNewPassword', () => {
    it('устанавливает новый пароль, удаляет запись reset и запрещает повторное использование токена', async () => {
      ...
      expect(userRepository.save).toHaveBeenCalledWith(expect.objectContaining({ password: 'hashed' }));
      ...
      resetRepository.findOne.mockResolvedValue(null);
      await expect(service.setNewPassword('t', 'again')).rejects.toBeInstanceOf(BadRequestException);
    });

    it('ошибка: недействительный токен', async () => {
      resetRepository.findOne.mockResolvedValue(null);
      await expect(service.setNewPassword('bad', 'x')).rejects.toBeInstanceOf(BadRequestException);
    });

    it('ошибка: пользователь не найден', async () => {
      resetRepository.findOne.mockResolvedValue({ token: 't', email: 'a@a.a', isVerified: true });
      userRepository.findOne.mockResolvedValue(null);
      await expect(service.setNewPassword('t', 'x')).rejects.toBeInstanceOf(BadRequestException);
    });
  });
});
