...
export class PromocodesService implements IPromocodeService {
  ...
  async create(
    ...
    if (existingPromocode) {
      throw new ConflictException(PROMOCODES_CONSTANTS.ERRORS.CODE_ALREADY_EXISTS);
    }
  ...
  async validateAndApply(
    ...
    if (!code || code.trim().length === 0) {
      return { isValid: false, message: PROMOCODES_CONSTANTS.ERRORS.INVALID_CODE };
    }

    if (orderAmount <= 0) {
      return { isValid: false, message: PROMOCODES_CONSTANTS.ERRORS.INVALID_ORDER_AMOUNT };
    }

    const promocode = await this.promocodesRepository.findActiveByCode(code);

    if (!promocode) {
      return { isValid: false, message: PROMOCODES_CONSTANTS.ERRORS.PROMOCODE_NOT_FOUND };
    }

    // Проверяем срок действия
    if (promocode.expiresAt && promocode.expiresAt < new Date()) {
      return { isValid: false, message: PROMOCODES_CONSTANTS.ERRORS.PROMOCODE_EXPIRED };
    }
    ...
    if (promocode.minOrderAmount && orderAmount < promocode.minOrderAmount) {
      return { 
        isValid: false, 
        message: PROMOCODES_CONSTANTS.ERRORS.MIN_ORDER_AMOUNT(promocode.minOrderAmount) 
      };
    }

    // Проверяем лимит использования
    if (promocode.maxUsage && promocode.currentUsage >= promocode.maxUsage) {
      return { isValid: false, message: PROMOCODES_CONSTANTS.ERRORS.USAGE_LIMIT_EXCEEDED };
    }

    // Проверяем, что скидка не больше 100%
    if (promocode.discount > 100) {
      return { isValid: false, message: PROMOCODES_CONSTANTS.ERRORS.INVALID_DISCOUNT };
    }
  ...
  async getAllActive(page: number = 1, limit: number = 20): Promise<{
  ...
  async searchPromocodes(
    ...
    if (!query || query.trim().length < PROMOCODES_CONSTANTS.MIN_SEARCH_LENGTH) {
      throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.SEARCH_TOO_SHORT);
    }
  ...
  async deactivate(code: string): Promise<void> {
    ...
    if (!promocode) {
      throw new NotFoundException(PROMOCODES_CONSTANTS.ERRORS.PROMOCODE_NOT_FOUND);
    }
  ...
  async deactivateMultiple(codes: string[]): Promise<{ deactivated: number; failed: number }> {
    if (!codes.length) {
      throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.NO_CODES_PROVIDED);
    }

    if (codes.length > PROMOCODES_CONSTANTS.MAX_BATCH_SIZE) {
      throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.BATCH_SIZE_EXCEEDED);
    }
  ...
  async update(
    ...
    const updatedPromocode = await this.promocodesRepository.updatePromocode(code, updates);
  ...
  private validatePromocodeData(
    ...
    if (options?.minOrderAmount !== undefined) {
      if (options.minOrderAmount < 0) {
        throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.INVALID_MIN_ORDER_AMOUNT);
      }
    }

    if (options?.expiresAt !== undefined) {
      if (options.expiresAt <= new Date()) {
        throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.INVALID_EXPIRY_DATE);
      }
    }
  }
  ...
  private validateDiscount(discount: number): void {
    if (discount <= 0 || discount > 100) {
      throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.INVALID_DISCOUNT);
    }
  }
  ...
  private validateMaxUsage(maxUsage: number, currentUsage: number = 0): void {
    if (maxUsage <= 0) {
      throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.INVALID_MAX_USAGE);
    }

    if (currentUsage > maxUsage) {
      throw new BadRequestException(PROMOCODES_CONSTANTS.ERRORS.CURRENT_USAGE_EXCEEDS_MAX);
    }
  }
