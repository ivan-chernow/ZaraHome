...
describe('RegistrationController (unit)', () => {
  ...
  describe('initiate', () => {
    it('инициация регистрации', async () => {
      ...
      expect(registrationService.initiateRegistration).toHaveBeenCalledWith('test@test.com');
      expect(responseService.success).toHaveBeenCalledWith(undefined, 'Код подтверждения отправлен на email test@test.com');
  ...
  describe('verifyCode', () => {
    it('верификация кода', async () => {
      ...
      expect(registrationService.verifyByCode).toHaveBeenCalledWith('test@test.com', '123456');
      expect(responseService.success).toHaveBeenCalledWith({ sessionToken }, 'Код подтверждения валиден');
      expect(result).toEqual({ success: true, message: 'test' });
    });
  });
  ...
  describe('complete', () => {
    it('завершение регистрации', async () => {
      const completeData = { sessionToken: 'session-token', password: 'password' };
      ...
      expect(registrationService.completeRegistration).toHaveBeenCalledWith('session-token', 'password');
      expect(responseService.success).toHaveBeenCalledWith({ user }, 'Регистрация завершена для пользователя test@test.com');
      expect(result).toEqual({ success: true, message: 'test' });
    });
  });
});
